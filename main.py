    ...
    with torch.no_grad():
        for i in range(TEST_SIZE):
            for episode in test_loader:
                ...
                # Embedding - Data Shot
                emb1 = model(data_shot)

                # 1xN each
                # Averaging each 5 data
                temp1 = (emb1[0] + emb1[1] + emb1[2] + emb1[3] + emb1[4])/5
                temp2 = (emb1[5] + emb1[6] + emb1[7] + emb1[8] + emb1[9])/5
                temp3 = (emb1[10] + emb1[11] + emb1[12] + emb1[13] + emb1[14])/5
                temp4 = (emb1[15] + emb1[16] + emb1[17] + emb1[18] + emb1[19])/5
                temp5 = (emb1[20] + emb1[21] + emb1[22] + emb1[23] + emb1[24])/5
                
                # Combine the calculations.
                ovTemp = [temp1, temp2, temp3, temp4, temp5]
                # Convert list to matrix.
                ovTemp = torch.cat(ovTemp)
                theLength = temp5.size()[0]
                # Shaping the matrix into 5xN size.
                ovTemp = ovTemp.reshape((5,theLength))
...
def train(args):
    ...
    for i in range(TOTAL):
        for episode in data_loader:
            ...
            # Loss function, which "Prototypical Networks for Few-shot learning" has used.
            loss_fn = torch.nn.CrossEntropyLoss()
            # Scaler, outputs loss values.
